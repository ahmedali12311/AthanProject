...
func (u *UserRoleDB) GetRolesByUserID(userID uuid.UUID) ([]Role, error) {
	const query = `
        SELECT
            r.id,
            r.name
	...
	var roles []Role
	if err := u.db.Select(&roles, query, userID); err != nil {
...
func (u *UserRoleDB) HasRole(userID uuid.UUID, roleID int) (bool, error) {
	var count int
